...
def init():
    ...
    cursor = cnn.cursor();
    cursor.execute("create table if not exists stud(id integer primary key autoincrement,name text,price integer)")
    cursor.close();
    cnn.close();
init();
...
def insert_data():
    ...
    if(a>0 and b>0):
        cnn = sq.connect("MY.db");
        cursor = cnn.cursor();
        cursor.execute("insert into stud(name,price)values(?,?)",(name,price));
        txt1.delete(0,END);
        txt2.delete(0,END);
        txt1.focus();
        cnn.commit();
...
def search_data(event):
    ...
    if(search==""):
        ...
        cnn = sq.connect("MY.db");
        cursor = cnn.cursor();
        cursor.execute("SELECT * FROM stud WHERE id LIKE ? or name LIKE ? or price LIKE ?",(f"%{search}%", f"%{search}%", f"%{search}%"));
...
def delete_data():
    ...
    if(a>0):
        ...
        if(msg):
            ...
            cnn = sq.connect("MY.db");
            cursor = cnn.cursor();
            cursor.execute("delete from stud where id=(?)",(v1,));
            cnn.commit();
            cursor.close()
            cnn.close();
...
def update_data():
    ...
    if (a>0):
        ...
        def up_data():
            ...
            cnn = sq.connect("MY.db");
            cursor = cnn.cursor();
            cursor.execute("update stud set name=(?),price=(?) where id=(?)",(name,price,v[0]));
            cnn.commit();
...
gp3 = LabelFrame(root,text="All",width="200",height="500")
...
btn2 = Button(gp2,text="Search",command=search_data);
